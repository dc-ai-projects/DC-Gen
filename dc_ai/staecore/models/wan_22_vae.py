# Modified from https://github.com/Wan-Video/Wan2.2/blob/main/wan/modules/vae2_2.py.
        ...
        if self.mode == "downsample3d":
            if feat_cache is not None:
                ...
                if feat_cache[idx] is None:
                    ...
                    feat_cache[idx] = cache_x
                    feat_idx[0] += 1
            else:  # support directly encoding the entire video (results not correct, just to test efficiency)
                x = F.pad(x, (0, 0, 0, 0, 1, 0), mode="replicate")
                x = self.time_conv(x)
        return x
...
@dataclass
class Wan22VAEConfig(BaseSTAECoreModelConfig):
    vae_pth: Optional[str] = "assets/checkpoints/wan/Wan2.2-TI2V-5B/Wan2.2_VAE.pth"


class Wan22VAE(BaseSTAECoreModel):
    def __init__(self, cfg: Wan22VAEConfig):
        super().__init__(cfg)
    ...
    def decode(self, z):
        return self.model.decode(z).clamp_(-1, 1)
