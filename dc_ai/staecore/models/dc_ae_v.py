...
from ...models.nn.ops import IdentityLayer
from ...models.nn.ops_3d import (
...
@dataclass
class DCAEVConfig(BaseSTAECoreModelConfig):
    in_channels: int = 3
    ...
    temporal_divisor: Optional[int] = None
...
def build_downsample_block(
    ...
    if block_name in ["ConvPixelUnshuffle", "CausalConvPixelUnshuffle"]:
        ...
        kwargs = {}
        if len(cfg) >= 8:
            kwargs["spatial_padding_mode"] = cfg[6]
            kwargs["temporal_padding_mode"] = cfg[7]
        block = ConvPixelUnshuffleDownSampleLayer3d(
            in_channels=in_channels,
            out_channels=out_channels,
            kernel_size=(int(cfg[4]), int(cfg[3]), int(cfg[3])),
...
def build_block(block_type: str, channels: int, norm: Optional[str], act: Optional[str], zero_out: bool) -> nn.Module:
    ...
    if block_name in ["ResBlock3d", "CausalResBlock3d"]:
        ...
        kwargs = {}
        if len(cfg) >= 6:
            kwargs["spatial_padding_mode"] = cfg[4]
            kwargs["temporal_padding_mode"] = cfg[5]
        main_block = ResBlock3d(
            in_channels=channels,
            out_channels=channels,
            kernel_size=(int(cfg[2]), int(cfg[1]), int(cfg[1])),
    ...
    def forward(
        ...
        *,
        ...
        if feature_cache is not None:
            returned_feature_cache = {}
            info["feature_cache"] = returned_feature_cache
        x, feature_cache_project_in = self.project_in(
            x, feature_cache, feature_key + "project_in." if feature_key is not None else None
        )
        if feature_cache is not None:
            assert feature_cache_project_in is not None
            returned_feature_cache |= feature_cache_project_in
        for stage_id, stage in enumerate(self.stages):
            if len(stage.op_list) == 0:
                continue
            x, feature_cache_stage = stage(
                x, feature_cache, feature_key + f"stages.{stage_id}." if feature_key is not None else None
            )
            if feature_cache is not None:
                assert feature_cache_stage is not None
                returned_feature_cache |= feature_cache_stage
        x, feature_cache_project_out = self.project_out(
            x, feature_cache, feature_key + "project_out." if feature_key is not None else None
        )
        if feature_cache is not None:
            assert feature_cache_project_out is not None
            returned_feature_cache |= feature_cache_project_out
        return x, info
...
class DCAEV(BaseSTAECoreModel):
    def __init__(self, cfg: DCAEVConfig):
        super().__init__(cfg)
        self.cfg: DCAEVConfig
    ...
    def temporal_divisor(self) -> int:
        if self.cfg.temporal_divisor is not None:
            return self.cfg.temporal_divisor
    ...
    def _prepare_image_input(self, x: torch.Tensor) -> torch.Tensor:
        """Convert an image batch from [B, C, H, W] to [B, C, T, H, W]."""
        num_image_frames = self.temporal_compression_ratio if self.cfg.num_pad_frames == 0 else 1
        padded_num_frames = num_image_frames + self.cfg.num_pad_frames
        if padded_num_frames % self.temporal_compression_ratio != 0:
            raise ValueError(
                f"image input with {num_image_frames} frame(s), num_pad_frames={self.cfg.num_pad_frames}, "
                f"padded_num_frames={padded_num_frames}, which is not divisible by "
                f"temporal_compression_ratio={self.temporal_compression_ratio}"
            )
        return x.unsqueeze(2).repeat(1, 1, num_image_frames, 1, 1)
    ...
    def temporal_tiled_encode(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in tqdm(range(0, x.shape[2], overlap_size), desc="Tiled Encode", disable=not self.cfg.verbose):
            ...
            tile, _ = self.encoder(
                tile, feature_cache=feature_cache, feature_key=f"encoder." if self.cfg.use_feature_cache else None
            )
            row.append(tile)
        result_row = []
    ...
    def temporal_tiled_decode(self, z: torch.Tensor) -> torch.Tensor:
        ...
        for i in tqdm(range(0, z.shape[2], overlap_size), desc="Tiled Decode", disable=not self.cfg.verbose):
            ...
            decoded, _ = self.decoder(tile, feature_cache, f"decoder." if self.cfg.use_feature_cache else None)
            row.append(decoded)
        result_row = []
        for i, tile in enumerate(row):
            if i > 0:
...
def dc_ae_v_f32t4_chunk_causal(name: str, pretrained_path: str) -> DCAEVConfig:
    ...
    cfg_str = (
        ...
        f"spatial_tile_size={spatial_tile_size} temporal_divisor={chunk_size} "
