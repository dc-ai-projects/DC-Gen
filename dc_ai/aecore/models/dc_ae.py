...
# GlobalDownsampleStage and GlobalUpsampleStage are based on the global skip connection architecture introduced in
# Qwen-Image-VAE-2.0: https://arxiv.org/abs/2605.13565
class GlobalDownsampleStage(nn.Module):
    def __init__(self, main: list[nn.Module], shortcut: nn.Module):
        super().__init__()
        self.main = OpSequential(main)
...
class GlobalUpsampleStage(nn.Module):
    def __init__(self, main: list[nn.Module], shortcut: nn.Module):
        super().__init__()
        self.main = OpSequential(main)
        self.shortcut = shortcut
...
def build_downsample_block(
    block_type: str, in_channels: int, out_channels: int, shortcut: Optional[str], factor: int = 2
) -> nn.Module:
    if block_type == "Conv":
        block = ConvLayer(
            in_channels=in_channels,
    ...
    def __init__(self, cfg: EncoderConfig):
        ...
        for stage_id, (width, depth) in enumerate(zip(cfg.width_list, cfg.depth_list)):
            ...
            if stage_id < num_stages - 1 and depth > 0:
                ...
                use_global_downsample_shortcut = cfg.downsample_shortcut == "global_averaging"
                downsample_block = build_downsample_block(
                    block_type=cfg.downsample_block_type,
                    in_channels=width,
                    out_channels=out_channels,
                    shortcut=None if use_global_downsample_shortcut else cfg.downsample_shortcut,
                )
                stage.append(downsample_block)
                if use_global_downsample_shortcut:
                    shortcut_block = PixelUnshuffleChannelAveragingDownSampleLayer(
                        in_channels=width, out_channels=out_channels, factor=2
                    )
                    stage = [GlobalDownsampleStage(stage, shortcut_block)]
...
class Decoder(nn.Module):
    def __init__(self, cfg: DecoderConfig):
        ...
        for stage_id, (width, depth) in reversed(list(enumerate(zip(cfg.width_list, cfg.depth_list)))):
            stage = []
            if stage_id < num_stages - 1 and depth > 0:
                use_global_upsample_shortcut = cfg.upsample_shortcut == "global_duplicating"
                upsample_block = build_upsample_block(
                    ...
                    shortcut=None if use_global_upsample_shortcut else cfg.upsample_shortcut,
                )
            ...
            if stage_id < num_stages - 1 and depth > 0 and cfg.upsample_shortcut == "global_duplicating":
                shortcut_block = ChannelDuplicatingPixelShuffleUpSampleLayer(
                    in_channels=cfg.width_list[stage_id + 1],
                    out_channels=width,
                    factor=2,
                )
                stage = [GlobalUpsampleStage(stage, shortcut_block)]
...
def dc_ae_f32_1_5(name: str, pretrained_path: str) -> DCAEConfig:
    if name in [
        "dc-ae-1.5-f32c128",
        "dc-ae-1.5-f32c128-c16",
        "dc-ae-1.5-f32c128-c32",
        "dc-ae-1.5-f32c128-c48",
        "dc-ae-1.5-f32c128-c64",
        "dc-ae-1.5-f32c128-c80",
        "dc-ae-1.5-f32c128-c96",
        "dc-ae-1.5-f32c128-c112",
    ]:
        latent_channels = 128
    else:
        raise ValueError(f"model {name} is not supported")
