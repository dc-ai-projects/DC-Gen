...
class AECoreMixtureDataset(MixtureDataset):
    def __getitem__(self, index: dict[str, Any]) -> dict[str, Any]:
        sample = self.datasets[index["dataset_index"]][index["sample_index"], index["resolution"], index["seed"]]
...
class AECoreMixtureDataProvider(MixtureDataProvider):
    ...
    def collate_fn(self, batch):
        collated_batch = {}
        collated_batch["images"] = default_collate([item.pop("image") for item in batch])
        keys = set()
        for item in batch:
