...
class SingleAutoencoder(nn.Module):
    ...
    def build_model(cls, model_name: str):
        if model_name in REGISTERED_DCAE_MODEL:
            ...
            model: AutoencoderDC = AutoencoderDC.from_pretrained(
                f"{REGISTERED_DCAE_DIFFUSERS_MODEL[model_name]}/{model_name}"
            )
            spatial_compression_ratio = model.spatial_compression_ratio
            ...
            if model_name in ["stabilityai/sd-vae-ft-ema", "stabilityai/sdxl-vae"]:
                ...
                model = diffusers.models.AutoencoderKL.from_pretrained(
            ...
            latent_channels = model.config.latent_channels
            scaling_factor = model.config.scaling_factor
        elif model_name == "flux2-vae":
            import diffusers

            model = diffusers.models.AutoencoderKLFlux2.from_pretrained(
                "black-forest-labs/FLUX.2-dev", subfolder="vae", torch_dtype=torch.bfloat16
            )
            spatial_compression_ratio = 16
            latent_channels = model.config.latent_channels * 4
            shifting_factor = -model.bn.running_mean.float().numpy()
            scaling_factor = 1 / torch.sqrt(model.bn.running_var + model.config.batch_norm_eps).float().numpy()
        elif model_name == "qwen-vae":
            import diffusers

            model = diffusers.models.AutoencoderKLQwenImage.from_pretrained(
                "Qwen/Qwen-Image-Edit",
                subfolder="vae",
            )
            latent_channels = model.config.z_dim
            shifting_factor = -np.array(model.config.latents_mean)
            scaling_factor = 1.0 / np.array(model.config.latents_std)
            spatial_compression_ratio = 8
        else:
            raise ValueError(f"autoencoder {model_name} is not supported")

        if isinstance(scaling_factor, np.ndarray) and latent_channels < scaling_factor.shape[0]:
            print(f"warning: latent channels {latent_channels} fewer than scaling factor {scaling_factor.shape[0]}")
        if isinstance(shifting_factor, np.ndarray) and latent_channels < shifting_factor.shape[0]:
            print(f"warning: latent channels {latent_channels} fewer than shifting factor {shifting_factor.shape[0]}")
    ...
    def encode(self, x: torch.Tensor) -> torch.Tensor:
        ...
        elif self.model_name == "flux2-vae":
            latent = self.model.encode(x).latent_dist.mode()
            latent = F.pixel_unshuffle(latent, 2)
        elif self.model_name == "qwen-vae":
            latent = self.model.encode(x.unsqueeze(2)).latent_dist.mode().squeeze(2)
        else:
    ...
    def decode(self, latent: torch.Tensor) -> torch.Tensor:
        ...
        if self.model_name in REGISTERED_DCAE_MODEL or self.model_name in REGISTERED_SD_VAE_MODEL:
            ...
            latent = F.pixel_shuffle(latent, 2)
            y = self.model.decode(latent).sample
        elif self.model_name == "qwen-vae":
            y = self.model.decode(latent.unsqueeze(2), return_dict=False)[0].squeeze(
                2
            )  # Squeeze [B, C, 1, H, W] to [B, C, H, W]
        else:
            raise ValueError(f"autoencoder {self.model_name} is not supported")
        return y
...
class Autoencoder(nn.Module):
    ...
    def reconstruct_image(self, x: torch.Tensor, setting_index: int = 0) -> tuple[torch.Tensor, Any, dict]:
        latent = self.encode(x, setting_index)
        y = self.decode(latent, setting_index)
        return y, None, {"latent": latent}
