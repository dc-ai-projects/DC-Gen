...
from ..apps.metrics.lpips import CustomLPIPSStats, CustomLPIPSStatsConfig
...
class AECoreTrainerConfig(BaseTrainerConfig):
    ...
    adaptive_latent_channels_sample_ratio: Optional[tuple[float]] = None
    ...
    compute_custom_lpips: bool = False
    custom_lpips: CustomLPIPSStatsConfig = field(default_factory=CustomLPIPSStatsConfig)
    compute_clip_iqa: bool = True
    compute_clip_score: bool = True
    compute_image_reward: bool = False
    ...
    sample_max_channel_on_first_n_ranks: Optional[int] = None
...
class AECoreTrainer(BaseTrainer):
            ...
            if pretrained_path is None:
                ...
                possible_models[model_name] = (
                    lambda model_cfg_func=model_cfg_func, model_name=model_name, pretrained_path=pretrained_path: (
                        DCAE(model_cfg_func(model_name, pretrained_path))
                    )
    ...
    def evaluate_single(
        ...
        if self.cfg.compute_custom_lpips:
            custom_lpips = CustomLPIPSStats(self.cfg.custom_lpips, device)
        ...
        with tqdm(
            ...
            for _, batch in enumerate(data_provider.data_loader):
                # preprocessing
                images = batch["image"].cuda().to(self.amp_dtype)
                # forward
                with torch.autocast(device_type="cuda", dtype=self.amp_dtype, enabled=True):
                    if latent_channels is not None:
                        ...
                        output, _, info = model("reconstruct_image", images)
                ...
                if self.cfg.compute_custom_lpips:
                    custom_lpips.add_data(input_images, output_images)
                if self.cfg.compute_clip_iqa:
                    clip_iqa.update(output_images_uint8.float())
                if self.cfg.compute_clip_score and "vlm_caption" in batch:
                    clip_score.update(output_images_uint8, batch["vlm_caption"])
                if self.cfg.compute_image_reward and "vlm_caption" in batch:
                    image_reward.add_data(output_images_uint8, batch["vlm_caption"])
                if self.cfg.compute_self_knn and data_provider.cfg.name == "ImageNetEval":
                    latent: torch.Tensor = info["latent"]
                    class_label: torch.Tensor = batch["class_label"].cuda()
        ...
        if self.cfg.compute_custom_lpips:
            eval_info_dict.update(custom_lpips.compute())
        if self.cfg.compute_clip_iqa:
