...
# This file is modified from https://github.com/Vchitect/VBench/blob/master/vbench2_beta_i2v/camera_motion.py, with the following changes:
# - remove Visualizer and third_party dependencies
# - support distributed evaluation
# - support video name to motion type mapping
...
from vbench2_beta_i2v.utils import load_dimension_info, load_video
...
def camera_motion(camera, video_list, video_name_to_motion_type: Optional[dict[str, str]] = None):
    sim = []
    video_results = []
    diff_type_results = {
        "pan_left": [],
    ...
    for video_path in tqdm(video_list):
        if video_name_to_motion_type is not None:
            target_type = video_name_to_motion_type[os.path.basename(video_path)]
        else:
...
def compute_camera_motion(json_dir, device, submodules_list, **kwargs):
    ...
    all_results, diff_type_results, video_results = camera_motion(
        camera, video_list, video_name_to_motion_type=kwargs.get("video_name_to_motion_type")
    )
    if get_world_size() > 1:
