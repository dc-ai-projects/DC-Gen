...
from ...apps.utils.aspect_ratio import get_aspect_ratio_manager

__all__ = [
    "VideoGenCoreLatentTrainDataProviderConfig",
    "VideoGenCoreLatentDataset",
    "VideoGenCoreLatentTrainDataProvider",
]


@dataclass
class VideoGenCoreTrainDataProviderConfig(BaseDataProviderConfig):
...
@dataclass
class VideoGenCoreLatentTrainDataProviderConfig(VideoGenCoreTrainDataProviderConfig):
    pass


class VideoGenCoreLatentDataset(WebDataset):
    def __init__(
        self,
        meta_path: str,
        temperature: float,
    ...
    def __getitem__(self, index: tuple[int, int]) -> dict[str, Any]:
        ...
        if self.latent_ext == ".npz":
            npz_data_raw = np.load(sample[".npz"])
            npz_data = {name: npz_data_raw[name] for name in npz_data_raw.files}
            latent, ae_feature, vlm_feature = npz_data["latent"], npz_data["img_feat"], npz_data["clip_feat"]
        elif self.latent_ext == ".pth":
            latent, ae_feature, vlm_feature = (
                sample[".pth"]["latent"],
                sample[".pth"]["img_feat"],
                sample[".pth"]["clip_feat"],
            )
        else:
            raise ValueError(f"latent ext {self.latent_ext} not supported")
...
class VideoGenCoreTrainDataProvider(BaseDataProvider):
    def __init__(self, cfg: VideoGenCoreTrainDataProviderConfig):
        super().__init__(cfg)
...
class VideoGenCoreLatentTrainDataProvider(VideoGenCoreTrainDataProvider):
    def __init__(self, cfg: VideoGenCoreLatentTrainDataProviderConfig):
        self.aspect_ratio_manager = get_aspect_ratio_manager(cfg.resolution)

        super().__init__(cfg)
        self.cfg: VideoGenCoreLatentTrainDataProviderConfig

    def build_complete_dataset(self) -> Dataset:
        dataset = VideoGenCoreLatentDataset(
