...
from .base_train import ImageEditCoreLatentTrainDataProviderConfig
from .collection import possible_train_data_providers


@dataclass
class ImageEditCoreLatentMixtureDataProviderConfig(MixtureDataProviderConfig):
    name: str = "ImageEditCoreLatentMixture"
...
class ImageEditCoreLatentMixtureDataset(MixtureDataset):
...
class ImageEditCoreLatentMixtureSampler(MixtureSampler):
    def __init__(
        self,
        cfg: ImageEditCoreLatentMixtureDataProviderConfig,
        datasets: list[Dataset],
        samplers: list[DistributedRangedSampler],
    ):
        super().__init__(cfg, datasets, samplers)
        self.cfg: ImageEditCoreLatentMixtureDataProviderConfig


class ImageEditCoreLatentMixtureDataProvider(MixtureDataProvider):
    def __init__(self, cfg: ImageEditCoreLatentMixtureDataProviderConfig):
    ...
    def build_datasets_and_samplers(self) -> tuple[list[Dataset], list[DistributedRangedSampler]]:
        ...
        for data_provider_name in self.cfg.data_providers:
            if data_provider_name in possible_train_data_providers:
                ...
                if isinstance(data_provider_cfg, ImageEditCoreLatentTrainDataProviderConfig):
    ...
    def build_complete_dataset(self) -> ImageEditCoreLatentMixtureDataset:
        return ImageEditCoreLatentMixtureDataset(self.cfg, self.datasets)
    ...
    def build_sampler(self) -> MixtureAspectRatioBatchSampler:
        raw_sampler = ImageEditCoreLatentMixtureSampler(self.cfg, self.datasets, self.samplers)
    ...
    def collate_fn(self, batch):
        ...
        ae_features = default_collate([item["ae_features"] for item in batch])
        raw_images = default_collate([item["raw_images"] for item in batch])
        batch = {
            key: [item[key] for item in batch] for key in batch[0] if key not in ["images", "ae_features", "raw_images"]
        }
        batch["images"] = images
        batch["ae_features"] = ae_features
