...
__all__ = [
    "ImageEditCoreLatentTrainDataProviderConfig",
    "ImageEditCoreLatentDataset",
    "ImageEditCoreLatentTrainDataProvider",
]


@dataclass
class ImageEditCoreTrainDataProviderConfig(BaseDataProviderConfig):
...
@dataclass
class ImageEditCoreLatentTrainDataProviderConfig(ImageEditCoreTrainDataProviderConfig):
    data_ext: str = ".pth"
    image_ext: str = ".jpg"


class ImageEditCoreLatentDataset(WebDataset):
    def __init__(
        self,
        meta_path: str,
        temperature: float,
    ...
    def __getitem__(self, index: tuple[int, int]) -> dict[str, Any]:
        ...
        data = sample[self.data_ext]
        latent, ae_feature = data["latent"], data["ae_feature"]

        if self.image_ext in sample:
            raw_image = sample[self.image_ext]
        ...
        if self.image_ext in sample:
            return_data["raw_images"] = raw_image
...
class ImageEditCoreTrainDataProvider(BaseDataProvider):
    def __init__(self, cfg: ImageEditCoreTrainDataProviderConfig):
        super().__init__(cfg)
    ...
    def build_filtered_dataset(self, complete_dataset: Dataset, mask: bool | np.ndarray) -> Dataset:
        if mask is True:
            ...
            raise ValueError(f"mask {mask} is not supported for VideoGenCoreDataProvider")


class ImageEditCoreLatentTrainDataProvider(ImageEditCoreTrainDataProvider):
    def __init__(self, cfg: ImageEditCoreLatentTrainDataProviderConfig):
        if cfg.resolution == "512F32MS":
            ...
            raise ValueError(f"resolution {cfg.resolution} is not supported for VideoMSCrop")

        self.transform = self.build_size_transform(cfg)

        super().__init__(cfg)
        self.cfg: ImageEditCoreLatentTrainDataProviderConfig

    def build_size_transform(self, cfg: ImageEditCoreLatentTrainDataProviderConfig) -> Callable:
        if cfg.size_transform == "QwenImageResize":
            if cfg.resolution in ["512F32MS", "512F64MS"]:
                resolution = 512
            elif cfg.resolution in ["1024", "2048", "4096"]:
                resolution = int(cfg.resolution)
            else:
                raise ValueError(f"Resolution {cfg.resolution} is not supported")

            if cfg.resolution in ["512F32MS"]:
                total_compression_ratio = 32
            elif cfg.resolution in ["512F64MS", "1024", "2048"]:
    ...
    def build_complete_dataset(self) -> Dataset:
        dataset = ImageEditCoreLatentDataset(
