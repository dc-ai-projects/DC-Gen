...
from ..utils.dtype import get_dtype_from_str
...
class SingleVideoTarDataset(SingleTarDataset):
    def __init__(self, tar_path: str, video_ext: str, transform: Optional[Callable]):
        super().__init__(tar_path)
    ...
    def __getitem__(self, index: int):
        sample = super().__getitem__(index)
        video: VideoLoader = sample[self.video_ext]
        sample["raw_image"] = video.get_frames([0])[0]
        sample["raw_num_frames"] = video.get_frame_count()
        if self.transform is not None:
            video = self.transform(video)
        del sample[self.video_ext]
        sample["video"] = video
        return sample
...
class LatentArchivesGenerator(ArchivesGenerator):
    def __init__(self, cfg: LatentArchivesGeneratorConfig):
        ...
        if cfg.load_archive_format in ["tar", "zip", "parquet"]:
            ...
            if self.meta.get("path_format", "absolute") == "relative":
                for shard in self.shardlist:
                    shard["url"] = os.path.abspath(os.path.join(self.cfg.meta_path, shard["url"]))
        else:
    ...
    def get_save_path(self, index: int) -> str:
        if self.cfg.archive_format == "tar":
            ...
            relative_tar_path = os.path.relpath(
                os.path.abspath(original_tar_path), os.path.abspath(self.cfg.original_archive_dir)
            )
    ...
    def collate_fn(self, batch):
        ...
        for key in batch[0].keys():
            if key in ["image", "video", "input_image"]:  #  Input image for image editing
