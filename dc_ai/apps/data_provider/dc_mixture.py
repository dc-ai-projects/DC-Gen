...
from ..utils.dist import get_dist_rank
...
class MixtureSampler(Sampler):
    ...
    def cur_state_dict(self) -> dict[str, Any]:
        state_dict = {
            f"rank_{self.rank}": {
                ...
                "sync_generator_state": self.sync_generator.get_state(),
                "async_generator_state": self.async_generator.get_state(),
                "samplers_state": self.samplers_state.clone(),
            }
        }
        return state_dict
    ...
    def load_state_dict(self, state_dict: dict[str, Any]) -> None:
        state_dict_single_rank = state_dict[f"rank_{self.rank}"]

        self.cur_iters = state_dict_single_rank["cur_iters"]
        self.sync_generator.set_state(state_dict_single_rank["sync_generator_state"])
        self.async_generator.set_state(state_dict_single_rank["async_generator_state"])
        self.samplers_state = state_dict_single_rank["samplers_state"]
