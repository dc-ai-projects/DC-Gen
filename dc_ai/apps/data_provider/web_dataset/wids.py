...
def default_decoder(sample: Dict[str, Any], format: Optional[Union[bool, str]] = True):
    ...
    for key, stream in sample.items():
        ...
        if key.startswith("__"):
            ...
            sample[key] = torch.load(stream, weights_only=True)
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(shards, str):
            ...
            self.base = os.path.dirname(shards)
            self.spec = json.load(open(shards, "r", encoding="utf-8"))
            self.shards = self.spec.get("shardlist", [])
            if isinstance(shards, str) and self.spec.get("path_format", "absolute") == "relative":
                for shard in self.shards:
                    shard["url"] = os.path.abspath(os.path.join(shards, shard["url"]))
            self.dataset_name = self.spec.get("name") or hash_dataset_name(str(shards))
        elif isinstance(shards, list):
            self.base = None
            self.spec = {"shardlist": shards}
            self.shards = shards  # list of dict with keys `url`, `num_samples` and `filesize`
            self.dataset_name = hash_dataset_name(str(shards))
        else:
...
class WebDataset(Dataset):
    def __init__(
        ...
        transformations: str = "PIL",
    ):
        self.data_dir = osp.expanduser(data_dir) if data_dir is not None else None

        if meta_path is not None:
            assert os.path.exists(meta_path), f"meta path {meta_path} not found"
            self.meta_path = meta_path
        elif self.data_dir is not None and os.path.exists(os.path.join(self.data_dir, "wids-meta.json")):
            self.meta_path = os.path.join(self.data_dir, "wids-meta.json")
        else:
            raise RuntimeError(f"can not find a valid meta path for {data_dir=} and {meta_path=}")

        uuid = hashlib.sha256(self.meta_path.encode()).hexdigest()[:8]
        self.dataset = ShardListDataset(
            self.meta_path,
            cache_dir=osp.expanduser(f"~/.cache/wids_cache/{getpass.getuser()}-{uuid}"),
            transformations=transformations,
        )
