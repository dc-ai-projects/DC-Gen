...
class CLIPScoreStats:
    ...
    def format_check(self, images: Tensor) -> Tensor:
    ...
    def compute_batch(self, images: Tensor, text: list[str]) -> tuple[float, int]:
        ...
        images = self.format_check(images)
...
# Image-to-Image CLIP Score for Image Editing Task
class I2ICLIPScoreStats(CLIPScoreStats):
    def __init__(self):
        super().__init__()

    def compute_batch(self, images1: Tensor, images2: Tensor) -> tuple[float, int]:
        """Compute CLIP score on a batch of input and edited images.

        Args:
            images1, images2: [N, C, H, W]

        Returns:
            score: the sum of CLIP score
            n_samples: the number of samples
        """
        assert isinstance(images1, Tensor), f"Type of images {type(images1)} is not supported"
        assert isinstance(images2, Tensor), f"Type of images {type(images2)} is not supported"
        if images1.shape[0] != images2.shape[0]:
            raise ValueError(
                f"The number of input images and output images do not match, got {images1.shape[0]} and {images2.shape[0]}"
            )

        images1, images2 = self.format_check(images1), self.format_check(images2)

        logit_scale = self.model.logit_scale.exp()
        real_features = self.model.encode_image(images1.to(self.device))
        fake_features = self.model.encode_image(images2.to(self.device))
        ...
        return score.sum().item(), images1.shape[0]

    @torch.no_grad()
    def update(self, images1: Tensor, images2: Tensor) -> None:
        """Update CLIP score on a batch of images and text.

        Args:
            images1, images2: [N, C, H, W]
        """
        score, n_samples = self.compute_batch(images1, images2)
        self.score += score
        self.n_samples += n_samples
