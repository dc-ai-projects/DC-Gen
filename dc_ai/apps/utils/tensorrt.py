...
def get_tensorrt_cmd(onnx_export_path: str, result_path: str) -> str:
    return f"trtexec --iterations=1000 --duration=0 --fp16 --device=0 --onnx={onnx_export_path} > {result_path}"
...
def get_tensorrt_result(onnx_export_path: str, result_path: str) -> dict[str, Any]:
    ...
    if not result_loaded:
        cmd = get_tensorrt_cmd(onnx_export_path, result_path)
        print(cmd)
        os.popen(cmd).read()
        result = load_tensorrt_result(result_path)
