...
# Wan 2.1 and Wan 2.2 was introduced by the Wan Team in "Wan: Open and Advanced Large-Scale Video Generative Models", see https://arxiv.org/abs/2503.20314.
# The original implementation is by the Wan Team, licensed under the Apache License 2.0. See https://github.com/Wan-Video/Wan2.1 and https://github.com/Wan-Video/Wan2.2.
class WanAspectRatioResize:
    def __init__(
        self,
        resolution: Optional[str] = "480",
        total_spatial_compression_ratio: Optional[int] = None,  # autoencoder spatial compression ratio * patch size
    ) -> None:
        self.resolution = resolution
        self.total_spatial_compression_ratio = total_spatial_compression_ratio

    def get_max_area(self, resolution: str) -> int:
        if resolution in ["480", "480F32", "480F64"]:
            return 832 * 480
        elif resolution in ["720", "720F32", "720F64"]:
            return 1280 * 720
        elif resolution in ["1080", "1080F32", "1080F64"]:
            return 1920 * 1080
        elif resolution in ["2160", "2160F32", "2160F64"]:
            return 3840 * 2160
        else:
    ...
    def __call__(
        self, img: Image.Image, resolution: Optional[str] = None, total_spatial_compression_ratio: Optional[int] = None
    ) -> Image.Image:
        resolution = resolution or self.resolution
        total_spatial_compression_ratio = total_spatial_compression_ratio or self.total_spatial_compression_ratio
        assert resolution is not None and total_spatial_compression_ratio is not None
        ih, iw = img.height, img.width
        dh, dw = total_spatial_compression_ratio, total_spatial_compression_ratio
        max_area = self.get_max_area(resolution)
        ow, oh = self.best_output_size(iw, ih, dw, dh, max_area)

        img = torchvision_F.to_tensor(img).sub_(0.5).div_(0.5)
        img = torch.nn.functional.interpolate(img[None].cpu(), size=(oh, ow), mode="bicubic")

        return img


class WanAspectRatioResizeCrop(WanAspectRatioResize):
    def __init__(
        self,
        resolution: Optional[str] = "480",
        total_spatial_compression_ratio: Optional[int] = None,  # autoencoder spatial compression ratio * patch size
    ) -> None:
        super().__init__(resolution=resolution, total_spatial_compression_ratio=total_spatial_compression_ratio)
...
class QwenImageResize:
    def __init__(
        self,
        resolution: int = 1024,
        total_compression_ratio: Optional[int] = None,  # autoencoder spatial compression ratio * patch size
    ) -> None:
        self.resolution = resolution
        self.total_compression_ratio = total_compression_ratio

    def calculate_dimensions(self, ratio, total_compression_ratio: Optional[int] = None):
        total_compression_ratio = total_compression_ratio or self.total_compression_ratio
        width = math.sqrt(self.resolution * self.resolution * ratio)
        height = width / ratio

        width = round(width / total_compression_ratio) * total_compression_ratio
        height = round(height / total_compression_ratio) * total_compression_ratio
    ...
    def __call__(
        self,
        img: Image.Image,
        total_compression_ratio: Optional[int] = None,
    ) -> Image.Image:
        ih, iw = img.height, img.width
        ow, oh = self.calculate_dimensions(iw / ih, total_compression_ratio)
...
class MultiResolutionImageFolder(ImageFolder):
    ...
    def __getitem__(self, index: int | tuple[int, int] | tuple[int, int, int]) -> dict[str, Any]:
        ...
        image = self.transform(image)
        sample = {"image": image, "index": index, "class_label": class_label}
        if self.vlm_caption is not None:
            sample["vlm_caption"] = self.vlm_caption.iloc[index]["caption"]
        return sample
