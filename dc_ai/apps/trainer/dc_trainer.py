...
from ..utils.dtype import convert_to_dtype_recursive, get_dtype_from_str
...
class BaseTrainerConfig:
    ...
    fsdp_ignore_frozen_parameters: bool = False
    ...
    max_steps_this_run: Optional[int] = None
    clip_grad: Optional[float] = None
    save_samples_steps: Optional[int] = 1000
    save_checkpoint_steps: int = 1000
    keep_checkpoint_steps: Optional[int] = None
    eval_steps: int = 20000
    save_eval_checkpoint_steps: int = "${.eval_steps}"
    save_eval_checkpoint_content: str = "model"  # "model" (model and ema), "ema" (ema only)
    save_eval_checkpoint_dtype: Optional[str] = None
...
class BaseTrainer:
        ...
        if self.cfg.mode in ["train"]:
            ...
            if self.cfg.activation_checkpointing_mode is not None:
                if self.cfg.distributed_method == "DDP":
                    ...
                    raise ValueError(f"distributed method {self.cfg.distributed_method} is not supported")
    ...
    def setup_dist_env(self) -> None:
        ...
        if self.cfg.allow_tf32 is not None:
            torch.backends.cuda.matmul.allow_tf32 = self.cfg.allow_tf32
            torch.backends.cudnn.allow_tf32 = self.cfg.allow_tf32
        if self.cfg.matmul_allow_tf32 is not None:
            torch.backends.cuda.matmul.allow_tf32 = self.cfg.matmul_allow_tf32
    ...
    def get_fsdp_ignored_parameters(self) -> list[nn.Parameter]:
        ignored_parameters = []
        for sub_module in self.model.modules():
            if isinstance(sub_module, nn.modules.batchnorm._BatchNorm):
                for paramter in sub_module.parameters():
                    ignored_parameters.append(paramter)
        if self.cfg.fsdp_ignore_frozen_parameters:
            for parameter in self.model.parameters():
                if not parameter.requires_grad:
                    ignored_parameters.append(parameter)
    ...
    def setup_model_for_training(self):
        if is_dist_initialized():
            if self.cfg.distributed_method == "DDP":
                ...
                self.model = FullyShardedDataParallel(
                    ...
                    ignored_states=self.get_fsdp_ignored_parameters(),
                )
            elif self.cfg.distributed_method == "FSDPWrap":
                from .fsdp_wrap import fsdp_wrap

                self.model = fsdp_wrap(self.model, ignored_parameters=self.get_fsdp_ignored_parameters())
            else:
                raise ValueError(f"distributed method {self.cfg.distributed_method} is not supported")
        if self.cfg.ema_decay is not None:
            self.ema = EMA(
                self.model,
                self.cfg.ema_decay,
                self.cfg.distributed_method,
                self.cfg.ema_warmup_steps,
                device=self.cfg.ema_device,
            )
    ...
    def gather_train_data_provider_states(self, train_data_provider_states: dict) -> dict:
        torch.cuda.empty_cache()
        for key in train_data_provider_states:
            if self.cfg.distributed_method == "DDP":
                train_data_provider_states[key] = gather_dict(train_data_provider_states[key])
            elif self.cfg.distributed_method in ["FSDP", "FSDPWrap"]:
                pass
            else:
                raise ValueError(f"distributed method {self.cfg.distributed_method} is not supported")
    ...
    def get_train_states(self, content: str = "full", place_holder: bool = False) -> dict[str, Any]:
        train_states = {}
        ...
        if content in "full":
            ...
            train_data_provider_states = self.get_train_data_provider_states(place_holder)
            train_states.update(self.gather_train_data_provider_states(train_data_provider_states))
            ...
            if self.cfg.ema_decay is not None:
                train_states["ema_model_state_dict"] = self.ema.get_ema_model_state_dict(self.model)
            ...
            if self.cfg.distributed_method == "DDP":
                ...
                with FullyShardedDataParallel.summon_full_params(
                    self.model, writeback=False, rank0_only=True, offload_to_cpu=True
                ):
                ...
                train_states["model_state_dict"] = model_state_dict
            else:
                raise ValueError(f"distributed method {self.cfg.distributed_method} is not supported")
        elif content == "ema":
            assert self.cfg.ema_decay is not None
            train_states["ema_model_state_dict"] = self.ema.get_ema_model_state_dict(self.model)
        else:
            raise ValueError(f"checkpoint content {content} is not supported")
    ...
    def save_model(self, model_name: str, content: str = "full", overwrite: bool = True) -> None:
        if model_name != "checkpoint":
            model_path = os.path.join(self.cfg.checkpoint_dir, f"{model_name}.pt")
            if not overwrite:
                model_exists = broadcast_object(os.path.isfile(model_path) if is_master() else False)
                if model_exists:
                    self.print_and_train_log(f"model {model_path} already exists, skip saving\n", flush=True)
                    return

        train_states = self.get_train_states(content)
        if model_name == "checkpoint":
            assert content == "full"

            # directly saving to keep_checkpoint is much faster than copying
            if self.cfg.keep_checkpoint_steps is not None and self.global_step % self.cfg.keep_checkpoint_steps == 0:
                keep_model_path = os.path.join(
                    self.cfg.checkpoint_dir, self.model_name_to_model_file_name(f"checkpoint_{self.global_step}")
                )
                if self.cfg.distributed_method == "DDP":
                    if is_master():
                        torch.save(train_states, keep_model_path)
                elif self.cfg.distributed_method in ["FSDP", "FSDPWrap"]:
                    if is_master():
                        shutil.rmtree(keep_model_path, ignore_errors=True)
                    dist_barrier()
                    torch.distributed.checkpoint.save(train_states, checkpoint_id=keep_model_path)
                else:
                    raise ValueError(f"distributed method {self.cfg.distributed_method} is not supported")
                dist_barrier()

            # first save to `checkpoint_`, then move to `checkpoint` to avoid partially saved checkpoints
            ...
            dist_barrier()
            if is_master():
                with open(self.latest_file_path, "w", encoding="utf-8") as f:
                    f.write("checkpoint_")
                ...
                shutil.move(model_path_, model_path)
                with open(self.latest_file_path, "w", encoding="utf-8") as f:
                    f.write("checkpoint")
        else:
            temp_model_path = f"{model_path}.tmp"
            if self.cfg.save_eval_checkpoint_dtype is not None:
                train_states = convert_to_dtype_recursive(
                    train_states, get_dtype_from_str(self.cfg.save_eval_checkpoint_dtype)
                )
            if is_master():
                try:
                    torch.save(train_states, temp_model_path)
    ...
    def process_grad_norm(self, optimizer: torch.optim.Optimizer) -> torch.Tensor:
        if self.cfg.clip_grad is not None:
            if self.cfg.distributed_method == "DDP":
                ...
                # Be careful! We can only clip grad for the entire model, instead of a specific optimizer
                grad_norm = self.model.clip_grad_norm_(self.cfg.clip_grad)
            else:
                raise ValueError(f"distributed method {self.cfg.distributed_method} is not supported")
        else:
            ...
            if self.cfg.distributed_method == "DDP":
                ...
                grad_norm = torch.norm(torch.tensor(sync_object(grad_norm)))
            else:
                raise ValueError(f"distributed method {self.cfg.distributed_method} is not supported")
        return grad_norm
    ...
    def train_step(self, batch: dict[str, Any]) -> tuple[dict[int, torch.Tensor], dict[str, Any]]:
        ...
        if self.cfg.gradient_accumulation_steps is None:
            ...
            for optimizer_idx, loss in loss_dict.items():
                ...
                if self.enable_grad_scaler:
                    ...
                    loss.backward()
                # gradient clip
                grad_norm = self.process_grad_norm(optimizer)
        ...
        if self.cfg.ema_decay is not None:
            self.ema.step(self.model, self.global_step)
    ...
    def after_step(
        ...
        for loss_key, loss_value in current_step_train_loss_dict.items():
            ...
            if loss_key not in average_loss_dict:
                average_loss_dict[loss_key] = AverageMeter()
            current_avg = average_loss_dict[loss_key].update(loss_value, batch_size)
            log_dict[f"train/{loss_key}"] = current_avg
        postfix_dict["global_step"] = str(self.global_step)
        for i, lr_scheduler in enumerate(self.lr_schedulers):
        ...
        with tqdm(
            ...
            while True:
                if self.global_step > 0 and self.global_step % self.cfg.eval_steps == 0:
                    # replace model weight with ema for evaluation
                    if self.cfg.ema_decay is not None:
                        self.ema.store(self.model)
                        self.ema.copy_to(self.model)

                    eval_info_dict = self.evaluate(self.global_step, model=self.model, f_log=self.train_log)
                    self.restore_training_component_placement()
                    ...
                    # restore model weight to continue training
                    if self.cfg.ema_decay is not None:
                    ...
                    if is_best_metric:
                        self.save_model("best", "ema" if self.cfg.ema_decay is not None else "model")

                    self.cache_log({"eval/" + key: value for key, value in eval_info_dict.items()})
                    self.log()
                    self.model.train()

                if self.global_step > 0 and self.global_step % self.cfg.save_eval_checkpoint_steps == 0:
                    self.save_model(f"step_{self.global_step}", self.cfg.save_eval_checkpoint_content, overwrite=False)
                ...
                if self.cfg.max_steps is not None and self.global_step >= self.cfg.max_steps:
                    ...
                    break
                if self.cfg.max_steps_this_run is not None and steps_this_run >= self.cfg.max_steps_this_run:
                    self.print_and_train_log(
                        f"max steps this run {self.cfg.max_steps_this_run} reached, terminate training\n", flush=True
                    )
                    break
                ...
                info["step_time"] = time.time() - step_start_time
