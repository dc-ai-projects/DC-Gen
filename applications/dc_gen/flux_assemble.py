...
from dc_ai.t2icore.models.flux import Flux, FluxConfig
...
@dataclass
class FluxAssembleConfig(FluxConfig):
...
def assemble_checkpoint(cfg: FluxAssembleConfig) -> Flux:
    target_model = Flux(cfg)

    # load backbone weights
    checkpoint = load_checkpoint(cfg.backbone_checkpoint_path)

    for key, target in zip(
        ["time_text_embed", "context_embedder", "transformer_blocks", "single_transformer_blocks", "norm_out"],
        [
            target_model.time_text_embed,
            target_model.context_embedder,
            target_model.transformer_blocks,
            target_model.single_transformer_blocks,
    ...
    if cfg.patch_embedding_checkpoint_path is not None:
        ...
        target_model.x_embedder.load_state_dict(x_embedder_weights)
...
def main():
    cfg: FluxAssembleConfig = OmegaConf.to_object(
        OmegaConf.merge(OmegaConf.structured(FluxAssembleConfig), OmegaConf.from_cli())
    )
