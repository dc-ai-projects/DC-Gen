...
from dc_ai.videogencore.models.wan_i2v import WanI2V, WanI2VConfig
...
@dataclass
class WanI2VAssembleConfig(WanI2VConfig):
...
def assemble_checkpoint(cfg: WanI2VAssembleConfig) -> WanI2V:
    target_model = WanI2V(cfg)
    ...
    if cfg.use_clip_feat:
        target_model.img_emb.load_state_dict(get_submodule_weights(checkpoint, "img_emb."))
        print(f"Load img_emb from {cfg.backbone_checkpoint_path}")
...
def main():
    cfg = get_config(WanI2VAssembleConfig)
